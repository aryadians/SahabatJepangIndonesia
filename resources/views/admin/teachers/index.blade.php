...
@section('page_title', 'Data Tenaga Pengajar (Sensei)')
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div>
                <p class="text-slate-400 text-xs font-bold uppercase tracking-wider">Total Pengajar</p>
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div>
                <p class="text-slate-400 text-xs font-bold uppercase tracking-wider">Aktif Mengajar</p>
        ...
        <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex items-center gap-4">
            ...
            <div>
                <p class="text-slate-400 text-xs font-bold uppercase tracking-wider">JLPT N1 / Native</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        @forelse($teachers as $tc)
            <div class="bg-white rounded-2xl p-5 border border-slate-200 shadow-sm flex flex-col justify-between hover:border-japan-600 transition group space-y-4">
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-start justify-between gap-3">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <span class="px-1.5 py-0.5 rounded bg-slate-100 text-[10px] font-mono font-bold text-slate-600">
                                    {{ $tc->nip }}
                                </span>
                                <h3 class="font-bold text-slate-900 text-xs mt-0.5 leading-tight">{{ $tc->name }}</h3>
                                <p class="text-[11px] font-bold text-japan-600 font-japanese">{{ $tc->romaji_name ?: '-' }}</p>
                            </div>
                        </div>
                    ...
                    <div class="p-3 rounded-xl bg-slate-50 border border-slate-100 space-y-1 text-xs">
                        <div>
                            ...
                            <p class="font-bold text-slate-900">{{ $tc->jlpt_level }}</p>
                        </div>
                        <div>
                            <span class="text-[10px] font-bold text-slate-400 uppercase">Spesialisasi:</span>
                            <p class="font-semibold text-japan-700 text-[11px]">{{ $tc->specialization }}</p>
                        </div>
                        @if($tc->japan_experience)
                            <div>
                                <span class="text-[10px] font-bold text-slate-400 uppercase">Pengalaman:</span>
                                <p class="text-slate-600 text-[10px]">{{ $tc->japan_experience }}</p>
                            </div>
                        @endif
                    </div>

                    <div class="flex items-center justify-between text-[11px] text-slate-500 pt-0.5">
                        <span><i data-lucide="phone" class="w-3 h-3 inline text-slate-400 mr-1"></i>{{ $tc->phone ?: '-' }}</span>
                        <span><i data-lucide="calendar" class="w-3 h-3 inline text-slate-400 mr-1"></i>{{ $tc->join_date ? $tc->join_date->format('M Y') : '-' }}</span>
                    </div>
        ...
        @empty
            <div class="col-span-3 py-12 text-center text-slate-400">
                ...
                <p class="font-bold text-slate-700">Belum ada data pengajar</p>
            </div>
        @endforelse
    </div>
